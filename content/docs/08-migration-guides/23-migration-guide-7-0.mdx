...
The equivalent stateless helpers live on the top-level `'ai'` export. Use them directly so the same transformation can be composed over any `fullStream` / `textStream` — not just a `streamText` result.
...
```tsx filename="AI SDK 7"
import { streamText, toUIMessageChunkStream } from 'ai';

const result = streamText({ model, prompt });

const uiStream = toUIMessageChunkStream({
...
pipeTextStreamToResponse({ response, textStream: result.textStream });
```
